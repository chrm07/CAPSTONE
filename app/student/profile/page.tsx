...
export default function ProfilePage() {
  ...
  useEffect(() => {
    if (user) {
      const p = user.profileData as any || {}
  ...
  const handleSaveInfo = async () => {
    ...
    try {
      ...
      await updateDoc(userRef, {
        profileData: { ...existingProfile, ...personalInfo }
      })
  ...
  const handleSaveAcademics = async () => {
    ...
    try {
      ...
      await updateDoc(userRef, {
        profileData: { ...existingProfile, ...educationInfo }
      })
              ...
              <CardContent className="p-6 md:p-8">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-8">
                  ...
                  <EditableSelectField 
                    ...
                    onChange={(v) => setEducationInfo({...educationInfo, course: v})} 
                  ...
                  {educationInfo.course === "Other" && (
                    <div className="animate-in fade-in zoom-in-95 duration-200">
                      <EditableField 
                        ...
                        value={educationInfo.customCourse} 
                        onChange={(v) => setEducationInfo({...educationInfo, customCourse: v})} 
                      />
                    </div>
                  )}
