...
export default function ScannerDashboardPage() {
  ...
  const handleSearch = async (e?: React.FormEvent) => {
    ...
    // Clean up input (handles exact QR payload like "BTS:USER_ID" or just the raw ID/Email)
    ...
    try {
      ...
      // Client-side filtering to match ID, Email, or Name
      const matchedApp = snapshot.docs.find(doc => {
        const data = doc.data()
        const searchLower = cleanQuery.toLowerCase()
        return (data.studentId === cleanQuery) ||
