...
import { 
  CalendarDays, Plus, Loader2, Banknote, Clock, MapPin, StopCircle, UploadCloud, RotateCcw, ChevronDown, ChevronUp, Search, Edit, Trash2, Archive, Users
} from "lucide-react"
    ...
    try {
      ...
      if (distributionType === "extension") {
          ...
          targetUserIds = appsSnap.docs
              .filter(d => !d.data().isClaimed)
              .map(d => d.data().studentId);
          targetUserIds = [...new Set(targetUserIds)]; 
      } else {
          const usersSnap = await getDocs(query(collection(db, "users"), where("role", "==", "student")));
          targetUserIds = usersSnap.docs.map(u => u.id);
      }

      const message = distributionType === "extension"
        ? `Financial distribution has been extended from ${formatDate(formData.startDate)} to ${formatDate(formData.endDate)}. Please proceed to claim your payout.`
        : `Financial distribution is scheduled from ${formatDate(formData.startDate)} to ${formatDate(formData.endDate)} (${formData.startTime}).`;
      ...
      <AdminLayout>
        <div className="max-w-7xl mx-auto space-y-8 animate-fade-in pb-12">
                      ...
                      {filteredBarangays.map((barangay, index) => (
                        <div key={barangay.id} className="bg-white border border-slate-200 rounded-2xl p-4 flex items-center justify-between gap-4 hover:border-emerald-400 hover:shadow-md transition-all">
                          <div className="flex items-center gap-4 overflow-hidden">
                            ...
                            <span className="font-black text-slate-900 text-xl uppercase tracking-tight truncate">{barangay.name}</span>
                          </div>
          ...
          <Dialog open={isAddModalOpen} onOpenChange={setIsAddModalOpen}>
            <DialogContent className="sm:max-w-5xl max-h-[95vh] flex flex-col p-0 rounded-3xl overflow-hidden border-0 shadow-2xl">
              <div className="h-2 bg-emerald-600 w-full shrink-0" />
              <DialogHeader className="px-8 py-6 border-b shrink-0 bg-white">
                <DialogTitle className="text-2xl font-black uppercase text-slate-800 tracking-tight flex items-center gap-2"><Banknote className="h-6 w-6 text-emerald-600" /> {distributionType === "extension" ? "Extend Financial Distribution" : "Open Financial Distribution"}</DialogTitle>
                <DialogDescription className="font-medium text-slate-500 text-base">{distributionType === "extension" ? "Set the new deadline. Notifications will only be sent to approved students who have not yet claimed their payout." : "Select the targeted barangays and define the distribution schedule for all students."}</DialogDescription>
              </DialogHeader>
              <div className="flex-1 overflow-y-auto px-8 py-6 bg-slate-50/50 flex flex-col gap-8">
                <div className="space-y-3 bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
                  <Label className="text-xs font-bold text-slate-700 uppercase tracking-wider">Distribution Type *</Label>
                  <Select value={distributionType} onValueChange={(v: any) => setDistributionType(v)}>
                    ...
                    <SelectContent className="rounded-xl border-slate-200">
                      <SelectItem value="regular" className="font-bold text-base">Financial Distribution (New Schedule)</SelectItem>
                      <SelectItem value="extension" className="font-bold text-base text-purple-700">Extension (Extend Existing Dates for Unclaimed Students)</SelectItem>
                    </SelectContent>
                  </Select>
                </div>
                  ...
                  <div className="space-y-4 flex flex-col animate-in fade-in slide-in-from-top-2 duration-300">
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                      ...
                      <div className="flex items-center gap-3">
                        <div className="relative w-full sm:w-64"><Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-slate-400" /><Input placeholder="Filter list..." value={barangaySelectionSearch} onChange={e => setBarangaySelectionSearch(e.target.value)} className="pl-9 h-10 rounded-xl bg-white border-slate-200" /></div>
                        <div className="flex gap-2 shrink-0">
                          <Button size="sm" variant="outline" onClick={() => setFormData({ ...formData, barangays: filteredSelectionBarangays.map(b => b.name) })} className="rounded-xl font-bold h-10 border-emerald-200 hover:bg-emerald-50 hover:text-emerald-700 text-emerald-600">Select Filtered</Button>
                          <Button size="sm" variant="outline" onClick={() => setFormData({ ...formData, barangays: [] })} className="rounded-xl font-bold h-10">Clear All</Button>
                        </div>
                      </div>
                    </div>
                    <div className="border border-slate-200 rounded-2xl p-4 bg-white overflow-y-auto shadow-inner max-h-[40vh] min-h-[250px]">
                      {filteredSelectionBarangays.length === 0 ? (
                         <div className="h-full flex items-center justify-center text-slate-400 py-10 font-bold">No barangays match your search.</div>
                      ) : (
                        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3">
                          {filteredSelectionBarangays.map((barangay, index) => (
                            <label key={barangay.id} className={`flex items-center gap-2 cursor-pointer p-3 rounded-xl border transition-all ${formData.barangays.includes(barangay.name) ? 'bg-emerald-50 border-emerald-200 shadow-sm' : 'bg-white border-slate-100 hover:border-slate-300 hover:bg-slate-50'}`}>
                              <input type="checkbox" checked={formData.barangays.includes(barangay.name)} onChange={() => toggleBarangay(barangay.name)} className="w-5 h-5 text-emerald-600 rounded-md border-slate-300 focus:ring-emerald-500 shrink-0" />
                              <span className="text-slate-400 font-black text-[10px] w-4 text-right shrink-0">{index + 1}.</span>
                              <span className={`text-sm font-bold uppercase truncate tracking-tight ${formData.barangays.includes(barangay.name) ? 'text-emerald-900' : 'text-slate-700'}`}>{barangay.name}</span>
                            </label>
                          ))}
                        </div>
                      )}
                    </div>
                    <div className="text-xs font-bold text-slate-500 uppercase tracking-widest text-right">{formData.barangays.length} Selected</div>
                  </div>
                )}
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 bg-white p-6 rounded-2xl border border-slate-200 shadow-sm animate-in fade-in duration-300">
                  <div className="space-y-2"><Label className="text-xs font-bold text-slate-700 uppercase tracking-wider">Start Date *</Label><Input type="date" value={formData.startDate} onChange={(e) => setFormData({ ...formData, startDate: e.target.value })} className="h-12 rounded-xl bg-slate-50 focus:bg-white border-slate-200 font-medium text-base" /></div>
                  {distributionType === "regular" && (
                    <div className="space-y-2"><Label className="text-xs font-bold text-slate-700 uppercase tracking-wider">Start Time *</Label><Input type="time" value={formData.startTime} onChange={(e) => setFormData({ ...formData, startTime: e.target.value })} className="h-12 rounded-xl bg-slate-50 focus:bg-white border-slate-200 font-medium text-base" /></div>
                  )}
                  <div className="space-y-2"><Label className="text-xs font-bold text-slate-700 uppercase tracking-wider">End Date *</Label><Input type="date" value={formData.endDate} onChange={(e) => setFormData({ ...formData, endDate: e.target.value })} className="h-12 rounded-xl bg-slate-50 focus:bg-white border-slate-200 font-medium text-base" /></div>
                  {distributionType === "regular" && (
                    <div className="space-y-2"><Label className="text-xs font-bold text-slate-700 uppercase tracking-wider">Distribution Amount (₱) *</Label><Input type="number" placeholder="e.g. 5000" value={formData.distributionAmount} onChange={(e) => setFormData({ ...formData, distributionAmount: e.target.value })} className="h-12 rounded-xl bg-slate-50 focus:bg-white border-slate-200 font-black text-emerald-700 text-xl" /></div>
                  )}
                </div>
              </div>
              <DialogFooter className="flex justify-end gap-3 px-8 py-5 border-t shrink-0 bg-white shadow-[0_-10px_20px_rgba(0,0,0,0.02)] z-10">
                <Button variant="outline" onClick={() => { setIsAddModalOpen(false); resetForm(); }} className="rounded-xl font-bold h-12 px-8 text-base">Cancel</Button>
                <Button onClick={handleOpenDistribution} className="bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl font-bold h-12 px-8 text-base shadow-md" disabled={isLoading}>{isLoading ? <Loader2 className="mr-2 h-5 w-5 animate-spin" /> : null} {distributionType === "extension" ? "Extend Schedule & Notify" : "Open Distribution"}</Button>
              </DialogFooter>
            </DialogContent>
          </Dialog>
            ...
            <DialogContent className="rounded-3xl border-0 shadow-2xl">
              ...
              <div className="space-y-4 py-4">
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-2"><Label className="text-xs font-bold text-slate-700 uppercase tracking-wider">Start Date</Label><Input type="date" value={subFormData.startDate} onChange={(e) => setSubFormData({...subFormData, startDate: e.target.value})} className="rounded-xl bg-slate-50" /></div>
                  <div className="space-y-2"><Label className="text-xs font-bold text-slate-700 uppercase tracking-wider">End Date</Label><Input type="date" value={subFormData.endDate} onChange={(e) => setSubFormData({...subFormData, endDate: e.target.value})} className="rounded-xl bg-slate-50" /></div>
                </div>
              </div>
              <DialogFooter>
